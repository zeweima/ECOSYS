...
C     ENDIF
      IF(BKDS(L,NY,NX).LE.ZERO.AND.DLYR(3,L,NY,NX).LT.ZERO)THEN
      DLYR(3,L,NY,NX)=0.0
      VOLW(L,NY,NX)=0.0
      VOLI(L,NY,NX)=0.0
      ENDIF
...
C
      IF(IPRC(NY,NX).LE.1.OR.IPRC(NY,NX).EQ.3)THEN
      DTBLX(NY,NX)=DTBLZ(NY,NX)
      ELSEIF(IPRC(NY,NX).EQ.2.OR.IPRC(NY,NX).EQ.4)THEN
...
C    3,AREA(3,NU(NY,NX),NY,NX),VOLWD(NY,NX)
