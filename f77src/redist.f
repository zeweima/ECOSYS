...
C
     ...
     3.AND.VOLI(L+1,NY,NX).LE.ZEROS(NY,NX))
     4.OR.(L.GT.0.AND.BKVL(L,NY,NX).LE.ZERO))
     5.AND.VOLW(L,NY,NX).GT.ZEROS(NY,NX)
     6.AND.VOLI(L,NY,NX).LE.ZEROS(NY,NX))THEN
      DO 1970 K=0,5
      IF(K.NE.3.AND.K.NE.4)THEN
C
C     MICROBIAL C,N,P
